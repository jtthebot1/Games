...
namespace Chess
{
	...
	public:
		// Iterator class to iterate over the board from top left to bottom right
		class iterator
		{
		private:
			Position pos;
		...
		iterator begin() const { return iterator(Position('A', '8')); }
		iterator end() const { return iterator(Position('A', '0')); }
		...
		int point_value(const bool &) const;

		//function to check if King is in check
		bool in_check(const bool &) const;
		...
		const std::map<Position, Piece *> &get_occ_const() const { return occ; }
